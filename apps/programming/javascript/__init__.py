import sys;from js2py import eval_js;import os;os.environ['PYGAME_HIDE_SUPPORT_PROMPT'] = "hide";import pygame;pygame.init();pygame.display.set_caption('python');font = pygame.font.Font(None, 32);clock = pygame.time.Clock();input_box = pygame.Rect(100, 100, 140, 32);out_box=pygame.Rect(100,200, 140, 32);from pygame_gui.elements import UIWindow;import pygame_gui
class js(UIWindow):
...
def load(manager, params):pos = params[0] if params is not None and len(params) > 0 else (100,100);js(pos, manager)
